...
- `api-worker` の `POST /v1/school/pop-proof` は、同一 claim に対する別端末からの再アクセスを短時間 idempotent に再利用し、PoP chain の不要な重複進行を防ぎます
...
- `grant_program` に `update_grant` 命令を追加し、`create_grant` の隠れ更新権限（`init_if_needed`）を `init` に変更。Grant パラメータの変更は明示的な `update_grant` 命令経由のみ可能になりました
- PoP の保証内容を signer-authenticated process receipt binding としてコード内コメントと README に明記しました
- `wene-mobile/server`（テストスタブ）と `api-worker`（本番）の辺りをコードコメントと README で明確化しました
...
| `api-worker/` | 本番バックエンド（Cloudflare Worker + Durable Object） | オフチェーン参加レシートの発行、admin/master 向け監査 API、readiness endpoint を提供します。本番・ Devnet の実稼働環境。 |
| `wene-mobile/` | user、admin、ローカル master フローで使う Expo アプリ | Web 向け画面は Cloudflare Pages にデプロイされています。 |
| `wene-mobile/server/` | テスト・ローカル開発専用スタブ（Node.js / Express） | **本番バックエンドではない。** `npm run test:server`（Vitest）とローカル開発に限定。ストレージはオンメモリのため翻発性で、PoP/監査 chain は実装なし。 |

> **バックエンド二重構造の意図**: `api-worker` は本番用途の唯一のバックエンド。`wene-mobile/server` はその API ルート層を Node.js 等価で実装したテストスタブであり、デプロイは一切せず単体テストのみのために存在します。
...
**PoP は「特定の signer が認証したプロセスレシートが、当該 claim に binding されている」こと——すなわち signer-authenticated process receipt binding——を保証する。**

- PoP は「オンチェーン canonical chain の連続性」を保証しない。
  chain head（`last_global_hash` / `last_stream_hash`）はベストエフォートで最新値に追従するが、並行 claim を拒否しない。
- 二重受給のための唯一のオンチェーンガードは receipt PDA の `init`（同一期間の 2 回目初期化は Anchor がエラーとする）である。
- v2 フォーマットの `audit_hash` に「オフチェーン参加レシートのコミットメント」を含めることは必須、0 フィールドは拒否する。
