...
- `POST /v1/school/pop-proof`
  - リクエスト: `{ eventId: string; confirmationCode: string; grant: string; claimer: string; periodIndex: string|number; expectedPrevHash?: string; expectedStreamPrevHash?: string }`
  - レスポンス: `{ signerPubkey, messageBase64, signatureBase64, auditHash, prevHash, streamPrevHash, entryHash, issuedAt }`
  - 同一 claim に対する別端末からの再アクセスは、fresh window 内では同じ proof を再利用して不要な重複発行を避ける
...
### Cost of Forgery Sybil Guard（任意）

Cost of Forgery 互換のリスク判定 API を登録/参加導線に組み込める:

- `COST_OF_FORGERY_ENABLED`: Cost of Forgery 判定を有効化（既定は `COST_OF_FORGERY_BASE_URL` 設定時のみ有効）
- `COST_OF_FORGERY_FAIL_CLOSED`: 判定API障害時に遮断するか（`true`=503で遮断, `false`=fail-open）
- `COST_OF_FORGERY_BASE_URL`: 判定APIのベースURL
- `COST_OF_FORGERY_VERIFY_PATH`: 判定APIパス（既定 `/v1/risk/score`。URL全体指定も可）
...
`POST /v1/school/pop-proof` はこの鍵で署名した PoP 証明を返し、クライアントは `claim_grant` 送信前に Ed25519 検証命令を付与する。同一 claim に対する別端末からの再アクセスは短時間 idempotent に同一 proof を再利用し、不要な重複発行を避ける。
