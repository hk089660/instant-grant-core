...
- `create_grant` から暗黙更新を除去し、明示的な `update_grant` 命令を分離（隠れパラメータ変更の防止）
- PoP v2（`audit_hash` コミット付き）を唯一の受付形式に限定（off-chain 参加証跡との binding を必須化）
- PoP chain head を best-effort 追従に変更し、並行 claim を拒否しない設計に明文化（二重受給ガードは receipt PDA init-dup に一本化）
- CI パイプラインを拡張（Anchor build + localnet integration test、API worker Vitest、mobile type check + server test）
...
- on-chain `ClaimReceipt` PDA で同一期間の claim を禁止（`init` による init-dup ガード）
...
- PoP chain head は best-effort 追従であり、並行 claim を拒否しない（唯一の二重受給ガードは receipt PDA init-dup）
...
- `create_grant` の再呼び出しにより、既存 grant のパラメータが意図せず上書きされる

対策（実装済み）:

- `create_grant` は `init` 制約により同一 PDA への再呼び出しを拒否
- パラメータ変更は `update_grant` 命令で明示的に行う（`has_one = authority` による権限検証付き）
- `update_grant` は `mint` / `vault` / `grant_id` を変更不可（PDA seed に含まれるため）
- 更新意図が監査ログ上で明確に分離される

関連箇所:

- `grant_program/programs/grant_program/src/lib.rs` — `create_grant` / `update_grant`
...
| コンポーネント | 監査状況 | 補足 |
| --- | --- | --- |
| `grant_program` | ❌ 未監査 | 外部監査ベンダー選定中。`update_grant` 分離・PoP v2 必須化により監査対象が明確化 |
| `wene-mobile` | ❌ 未監査 | モバイルセキュリティレビュー未着手 |
| `api-worker` | ❌ 未監査 | trust boundary の整理と外部確認が必要 |
| CI / 自動テスト | ✅ 一部対応 | Anchor localnet integration test、API worker Vitest、mobile server test を CI で実行 |
...
| レイヤー | テスト種別 | CI 統合 | 補足 |
| --- | --- | --- | --- |
| `grant_program` | Anchor localnet integration | ✅ | `anchor test --provider.cluster localnet` |
| `api-worker` | Vitest unit / integration | ✅ | `npm test` + `tsc --noEmit` |
| `wene-mobile` | Vitest server test | ✅ | `npm run test:server` + `tsc --noEmit` |
| lockfile policy | bash script | ✅ | `scripts/check-lockfile-policy.sh` |

## 外部保証計画（2026-04-27 更新）

| 項目 | 当初目標 | 現在の状況 |
| --- | --- | --- |
| 監査範囲の確定 | 2026-03-15 | 遅延。`update_grant` 追加と PoP v2 限定化により対象が再整理された |
| スマートコントラクト外部監査開始 | 2026-04-01 | 未着手。ベンダー選定を進行中 |
| 是正ステータスの公開 | 2026-04-30 | 監査開始に依存。再スケジュール予定 |
| モバイルセキュリティレビュー | 2026-04-15 | 未着手 |

次のアクション:

1. 外部監査ベンダーの選定と契約（2026 Q2 目標）
2. 監査範囲: `grant_program` 全命令 + `api-worker` trust boundary
3. 監査完了後に是正ステータスを本ドキュメントに反映
...
8. `update_grant` 操作の監視と通知（パラメータ変更の即時検知）
